...
class HetAgg(nn.Module):
    ...
    def node_het_agg(self, id_batch, node_type):
        ...
        b = [[]]
        for i in range(len(a_neigh_batch)):
            for j in range(len(a_neigh_batch[i])):
                b[0].append(a_neigh_batch[i][j])
        a_neigh_batch = np.array(b)
        a_agg_batch = self.node_neigh_agg(a_neigh_batch, 1)

        b = [[]]
        for i in range(len(p_neigh_batch)):
            for j in range(len(p_neigh_batch[i])):
                b[0].append(p_neigh_batch[i][j])
        p_neigh_batch = np.array(b)
        p_agg_batch = self.node_neigh_agg(p_neigh_batch, 2)

        b = [[]]
        for i in range(len(v_neigh_batch)):
            for j in range(len(v_neigh_batch[i])):
                b[0].append(v_neigh_batch[i][j])
        v_neigh_batch = np.array(b)
